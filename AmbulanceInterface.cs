    ...
    public partial class AmbulanceInterface : Form
    {
        ...
        private void LoadDataIntoComboBox()
        {
            try
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT DISTINCT District FROM AmbulanceData";

                    using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                    {
                        DataSet dataSet = new DataSet();
                        adapter.Fill(dataSet, "AmbulanceData");

                        comboBox1.DataSource = dataSet.Tables["AmbulanceData"];
        ...
        public void comboBoxDistrict_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (comboBox1.SelectedIndex != -1)
            {
                ...
                string query2 = "SELECT DISTINCT Thana FROM AmbulanceData WHERE District = @SelectedDistrict";
        ...
        private void AmbulanceInterface_Load(object sender, EventArgs e)
        {
            populate();
        ...
        private void UpdateAmbulanceView()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(selectedDistrict) && !string.IsNullOrEmpty(selectedThana))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, Con))
                    {
                        ...
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                        {
                            ...
                            {
                                AmbulanceView.DataSource = searchResultTable;
                            }
                            else
                            {
                                AmbulanceView.DataSource = null; // Clear the DataGridView
                            }
                        }
                    }
