...
namespace Shomvob
{
    public partial class TrainInterface : Form
    {
        ...
        private void LoadDataIntoComboBox()
        {
            try
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT DISTINCT Fromm FROM TrainInfoTbl";
                    string query2 = "SELECT DISTINCT Too FROM TrainInfoTbl";


                    using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
                    {
                        DataSet dataSet = new DataSet();
                        adapter.Fill(dataSet, "TrainInfoTbl");

                        comboBox1.DataSource = dataSet.Tables["TrainInfoTbl"];
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename='D:\4th Sem\Project Shomvob\DataBase\Shomvob.mdf';Integrated Security=True;Connect Timeout=30"))
                {
                    ...
                    using (SqlDataAdapter adapter = new SqlDataAdapter(query2, connection))
                    {
                        ...
                        comboBox2.DataSource = dataSet.Tables["TrainInfoTbl"];
        ...
        private void TrainInterface_Load(object sender, EventArgs e)
        {
        ...
        private void bunifuThinButton21_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(selectedFromm) && !string.IsNullOrEmpty(selectedToo))
                {
                    ...
                    string query = "SELECT * FROM TrainInfoTbl WHERE Fromm = @SelectedFromm AND Too = @SelectedToo";
